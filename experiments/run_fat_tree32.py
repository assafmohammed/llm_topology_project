...
def main() -> None:
    ...
    hop_matrix_path = results_dir / "fat_tree32_hop_matrix.csv"
    traffic_matrix_path = results_dir / "fat_tree32_traffic_matrix.csv"
    distribution_path = results_dir / "fat_tree32_active_hop_distribution.csv"
    traffic_npy_path = traffic_dir / "synthetic_fat_tree32_tp8_dp2_pp2.npy"

    hop_heatmap_path = results_dir / "fat_tree32_hop_heatmap.png"
    traffic_heatmap_path = results_dir / "fat_tree32_traffic_heatmap.png"
    ...
    plot_hop_heatmap(
        hop_matrix,
        hop_heatmap_path,
        title="Fat-Tree-32 GPU-to-GPU Hop Distance Heatmap",
        show_values=False,
    )
    ...
    plot_traffic_heatmap(
        ...
        title="Fat-Tree-32 Synthetic LLM-like Traffic Matrix",
        show_values=False,
    )

    print(fat_tree_summary(graph))
    ...
    print()
    print("Sanity checks:")
    print("GPU 0 -> GPU 7 same HBI:", fat_tree_hop_distance(0, 7, cfg), "hop")
    print("GPU 0 -> GPU 8 same local rank through rail:", fat_tree_hop_distance(0, 8, cfg), "hops")
    print("GPU 0 -> GPU 16 same local rank through rail:", fat_tree_hop_distance(0, 16, cfg), "hops")
    print(
        "GPU 0 -> GPU 9 different local rank across HBI domains:",
        fat_tree_hop_distance(0, 9, cfg),
        "hops",
    )
