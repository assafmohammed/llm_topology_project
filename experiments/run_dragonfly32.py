...
def main() -> None:
    ...
    graph = build_dragonfly_plus(cfg)
    ...
    hop_matrix_path = results_dir / "dragonfly32_hop_matrix.csv"
    traffic_matrix_path = results_dir / "dragonfly32_traffic_matrix.csv"
    distribution_path = results_dir / "dragonfly32_active_hop_distribution.csv"
    traffic_npy_path = traffic_dir / "synthetic_dragonfly32_tp8_dp2_pp2.npy"

    hop_heatmap_path = results_dir / "dragonfly32_hop_heatmap.png"
    traffic_heatmap_path = results_dir / "dragonfly32_traffic_heatmap.png"
    ...
    plot_hop_heatmap(
        hop_matrix,
        hop_heatmap_path,
        title="Dragonfly+-32 GPU-to-GPU Hop Distance Heatmap",
        show_values=False,
    )
    ...
    plot_traffic_heatmap(
        ...
        title="Dragonfly+-32 Synthetic LLM-like Traffic Matrix",
        show_values=False,
    )

    print(dragonfly_plus_summary(graph))
    ...
    print()
    print("Sanity checks:")
    print("GPU 0 -> GPU 7 same HBI:", dragonfly_hop_distance(0, 7, cfg), "hop")
    print(
        "GPU 0 -> GPU 8 same Dragonfly+ group / within same stage:",
        dragonfly_hop_distance(0, 8, cfg),
        "hops",
    )
    print(
        "GPU 0 -> GPU 16 different Dragonfly+ group / cross stage:",
        dragonfly_hop_distance(0, 16, cfg),
        "hops",
    )
    print(
        "GPU 0 -> GPU 24 different Dragonfly+ group:",
        dragonfly_hop_distance(0, 24, cfg),
        "hops",
    )
